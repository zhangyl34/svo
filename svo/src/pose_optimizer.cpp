    ...
    size_t& num_obs)
{
  ...
  for(auto it=frame->fts_.begin(); it!=frame->fts_.end(); ++it)
  {
    if((*it)->point == NULL)
      continue;
    // project2d(v): v.head<2>()/v[2]
    Vector2d e = vk::project2d((*it)->f)
               - vk::project2d(frame->T_f_w_ * (*it)->point->pos_);  // f 是相机坐标系下单位深度坐标
    e *= 1.0 / (1<<(*it)->level);
    errors.push_back(e.norm());
  }
  if(errors.empty())
    return;
  vk::robust_cost::MADScaleEstimator scale_estimator;  // median absolute deviation
  estimated_scale = scale_estimator.compute(errors);  // 1 / 0.6745 * getMedian(errors)

  num_obs = errors.size();
  chi2_vec_init.reserve(num_obs);
  chi2_vec_final.reserve(num_obs);
  double scale = estimated_scale;
  for(size_t iter=0; iter<n_iter; iter++)  // <10
  {
    // overwrite scale
    if(iter == 5)
  ...
  double reproj_thresh_scaled = reproj_thresh / frame->cam_->errorMultiplier2();  // 2/f
