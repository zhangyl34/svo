...
{
  SVO_INFO_STREAM("addSecondFrame.");
  // 调用 opencv 函数，光流法追踪特征点
  trackKlt(frame_ref_, frame_cur, px_ref_, px_cur_, f_ref_, f_cur_, disparities_);
  SVO_INFO_STREAM("Init: KLT tracked "<< disparities_.size() <<" features");

  if(disparities_.size() < Config::initMinTracked())  // default: 50
    return FAILURE;

  double disparity = vk::getMedian(disparities_);  // 取中位数
  SVO_INFO_STREAM("Init: KLT "<<disparity<<"px average disparity.");  // default: 50
  if(disparity < Config::initMinDisparity())  // 视角没怎么变化
    return NO_KEYFRAME;
  ...
  // Rescale the map such that the mean scene depth is equal to the sp le
  vector<double> depth_vec;
  for(size_t i=0; i<xyz_in_cur_.size(); ++i)
    depth_vec.push_back((xyz_in_cur_[i]).z());
  double scene_depth_median = vk::getMedian(depth_vec);  // 取中位数
  double scale = Config::mapScale()/scene_depth_median;
  frame_cur->T_f_w_ = T_cur_from_ref_ * frame_ref_->T_f_w_;
  // frame_ref_->pos() = [0;0;0]
  SVO_INFO_STREAM(std::to_string(Config::mapScale()));
