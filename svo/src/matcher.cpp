...
void getWarpMatrixAffine(
    ...
    const Vector3d& f_ref,  // 相机坐标系下的单位深度 3d 坐标
...
void Matcher::createPatchFromPatchWithBorder()
{
  ...
  for(int y=1; y<patch_size_+1; ++y, ref_patch_ptr += patch_size_)  // patch_size_=8
  {
...
bool Matcher::findMatchDirect(
    ...
    Vector2d& px_cur)
{
  // 返回能观测到 pt 且与 cur_frame 视角最接近的关键帧
  ...
  // 对 ref_frame 上的矩形 patch 执行仿射变换，记录在 A_cur_ref_ 上
  ...
  // 对 A_cur_ref_ 取逆，对 cur_frame 上的矩形 patch 的像素逐一读取 ref_frame 上的对应值，填入 patch_with_border_
  ...
  // 将 patch_with_border_ 填入 patch_
  ...
  bool success = false;
  if(ref_ftr_->type == Feature::EDGELET) {  // Feature::CORNER
