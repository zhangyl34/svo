...
SparseImgAlign::SparseImgAlign(
    ...
    Method method, bool display, bool verbose) :
        ...
        max_level_(max_level),  // 4
        min_level_(min_level)   // 2
{
  n_iter_ = n_iter;        // 30
  n_iter_init_ = n_iter_;  // 30
  method_ = method;        // SparseImgAlign::GaussNewton
  verbose_ = verbose;      // false
  eps_ = 0.000001;
}
...
size_t SparseImgAlign::run(FramePtr ref_frame, FramePtr cur_frame)
{
  ...
  ref_patch_cache_ = cv::Mat(ref_frame_->fts_.size(), patch_area_, CV_32F);  // ftsSize * 16，双线性插值 4x4 特征区域的亮度
  jacobian_cache_.resize(Eigen::NoChange, ref_patch_cache_.rows*patch_area_);  // 6 行
  visible_fts_.resize(ref_patch_cache_.rows, false); // TODO: should it be reset at each level?

  SE3 T_cur_from_ref(cur_frame_->T_f_w_ * ref_frame_->T_f_w_.inverse());  // 初始化为 I

  // 迭代三次
  for(level_=max_level_; level_>=min_level_; --level_)  // 4, 3, 2
  {
    mu_ = 0.1;
...
{
  const int border = patch_halfsize_+1;  // 3
  const cv::Mat& ref_img = ref_frame_->img_pyr_.at(level_);
  const int stride = ref_img.cols;       // 图像列数
  const float scale = 1.0f/(1<<level_);
  const Vector3d ref_pos = ref_frame_->pos();  // 世界坐标系下的相机坐标
      ...
      it!=ite; ++it, ++feature_counter, ++visiblity_it)
  {
    ...
    *visiblity_it = true;
    ...
    const Vector3d xyz_ref((*it)->f*depth);  // ref 相机坐标系下的坐标
    ...
    bool compute_weight_scale)
{
  ...
  if(linearize_system && display_)  // true && false
    resimg_ = cv::Mat(cur_img.size(), CV_32F, cv::Scalar(0));

  if(have_ref_patch_cache_ == false)
    precomputeReferencePatches();  // 加载 fts_size x 16 的 ref_patch_cache_

  // compute the weights on the first iteration
  std::vector<float> errors;
  if(compute_weight_scale)  // false
      ...
      ++it, ++feature_counter, ++visiblity_it)
  {
    ...
    const double depth = ((*it)->point->pos_ - ref_pos).norm();
    const Vector3d xyz_ref((*it)->f*depth);            // 在 ref 相机坐标系下的坐标
    const Vector3d xyz_cur(T_cur_from_ref * xyz_ref);  // 在 cur 相机坐标系下的坐标
  ...
  // compute the weights on the first iteration
  if(compute_weight_scale && iter_ == 0)  // false
...
{
  x_ = H_.ldlt().solve(Jres_);  // 求解线性方程 H_ * x_ = Jres_
