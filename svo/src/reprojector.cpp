    ...
    std::vector< std::pair<FramePtr,std::size_t> >& overlap_kfs)
{
  ...
  // 遍历关键帧，如果关键帧中有能被当前帧观测到的特征点，则记录该关键帧以及两帧之间的位移。
  map_.getCloseKeyframes(frame, close_kfs);
  // 根据距离排序
  close_kfs.sort(boost::bind(&std::pair<FramePtr, double>::second, _1) <
                 boost::bind(&std::pair<FramePtr, double>::second, _2));

  // 遍历 n 近邻的 close_kfs 上的特征点，记录特征点落在 grid_ 的哪个 cell 上
  size_t n = 0;
      ...
      it_frame!=ite_frame && n<options_.max_n_kfs; ++it_frame, ++n)
  {
        ...
        it_ftr!=ite_ftr; ++it_ftr)
    {
      ...
      // 不同 ref_frame 上相同的特征点，只处理一次。
      if((*it_ftr)->point->last_projected_kf_id_ == frame->id_)
        continue;
      (*it_ftr)->point->last_projected_kf_id_ = frame->id_;
      // 判断 point 是否重投影到 frame 上，如果是，将 grid_ 与 point 建立联系
  ...
  SVO_START_TIMER("reproject_candidates");
  {
    ...
    while(it!=map_.point_candidates_.candidates_.end()) {
      if(!reprojectPoint(frame, it->first)) {
        ...
        // 如果 map_ 中的特征点多次无法投影到当前帧中，则该特征点将被移除 map_
  ...
  for(size_t i=0; i<grid_.cells.size(); ++i) {
    ...
    if(reprojectCell(*grid_.cells.at(grid_.cell_order[i]), frame))
      ++n_matches_;
    if(n_matches_ > (size_t) Config::maxFts())  // >120
      break;
  }
...
bool Reprojector::reprojectCell(Cell& cell, FramePtr frame)
{
  ...
  while(it!=cell.end()) {
    ...
    if(options_.find_match_direct)  // true
      found_match = matcher_.findMatchDirect(*it->pt, *frame, it->px);  // 校正 it->px
    if(!found_match) {
    ...
    if(matcher_.ref_ftr_->type == Feature::EDGELET) {  // Feature::CORNER
